...
from To_Do.tasks.forms import (
    CreateTaskForm,
    UpdateTaskForm,
    NoteForm,
    TaskCollaboratorForm,
    NoteFormUpdate,
)
...
def task(task_id):
    ...
    previous_task = (
        Task.query.filter(Task.id < task_id, Task.user_id == current_user.id)
...
def new_collaborator(task_id):
    ...
    if form.validate_on_submit():
        ...
        existing_collaborator = TaskCollaborator.query.filter_by(
    ...
    return render_template(
        "create_collaborator.html", title="Add Collaborator", form=form
    )
...
def remove_note(note_id):
    ...
    # Find the previous task
    previous_task = (
        Task.query.filter(Task.id < note.task_id, Task.user_id == current_user.id)
        .order_by(Task.id.desc())
        .first()
    )
